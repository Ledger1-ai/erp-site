...
export function BasaltHero() {
    return (
        <section id="hero" className="relative min-h-screen flex items-center justify-center px-6 pt-24 overflow-hidden">
            {/* Background Effects (Mocking BasaltWaveConwayBackground for now, or using a simple gradient if not ported yet) */}
            {/* We rely on the global body background, but can add a specific hero accent if needed */}
            ...
            <div className="max-w-5xl mx-auto text-center z-10">
                ...
                {/* Mission Statement */}
                <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-12 leading-relaxed opacity-0 animate-fadeInUp stagger-3">
                    Focus on the features that keep you competitive in this new era.
                    Be excited about business again. The first ERP that doesn't just manage your data—it actively works for you.
                    From finding prospects to closing deals with voice AI, BasaltERP is your unfair advantage.
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-6 opacity-0 animate-fadeInUp stagger-4">
                    ...
                    <Link
                        href="/register"
                        className="h-12 px-8 flex items-center justify-center text-sm font-bold tracking-wider rounded-[10px] border border-white/20 text-foreground hover:bg-white/10 hover:border-white/40 transition-all duration-300 backdrop-blur-sm"
                    >
                        GET STARTED FREE
                    </Link>
                </div>
                ...
                <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-8 opacity-0 animate-fadeInUp stagger-4">
                    {[
                        ...
                        <div key={stat.label} className="text-center group">
                            <div className="text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-foreground to-muted-foreground group-hover:from-cyan-400 group-hover:to-teal-500 transition-all duration-300 mb-1">
                                {stat.value}
                            </div>
            ...
            <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 opacity-0 animate-fadeInUp stagger-5">
                <Link href="#features" className="flex flex-col items-center gap-2 text-muted-foreground hover:text-cyan-500 transition-colors">
